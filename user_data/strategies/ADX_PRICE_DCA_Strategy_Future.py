...
class DMIPRICEDCAStrategyFuture(IStrategy):
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if trade.entry_side == 'buy' : 
            if last_candle_price <= last_order_price * self.downGridPercent:
                try:
        ...
        if trade.entry_side == 'sell' : 
            if last_candle_price >= last_order_price * self.upGridPercent:
                try:
                    # This returns first order stake size
                    stake_amount = filled_entries[0].stake_amount
                    return stake_amount
